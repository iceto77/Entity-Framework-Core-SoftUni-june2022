namespace Theatre
{
    ...
    public class StartUp
    {
        public static void Main(string[] args)
        {
            ...
            //Problem01  - закоментирай другите две за джъдж
            //ResetDatabase(context, shouldDropDatabase: true);

            var projectDir = GetProjectDirectory();

            //Problem02 - закоментирай другите две за джъдж
            //ImportEntities(context, projectDir + @"Datasets/", projectDir + @"ImportResults/");

            //Problem03 - закоментирай другите две за джъдж
        ...
        {
            var plays = DataProcessor.Deserializer.ImportPlays(context, File.ReadAllText(baseDir + "plays.xml"));
            PrintAndExportEntityToFile(plays, exportDir + "Actual Result - ImportTheatresTickets.txt");

            var casts = DataProcessor.Deserializer.ImportCasts(context, File.ReadAllText(baseDir + "casts.xml"));
            PrintAndExportEntityToFile(casts, exportDir + "Actual Result - ImportCasts.txt");

            var theatersAndTickets = DataProcessor.Deserializer.ImportTtheatersTickets(context, File.ReadAllText(baseDir + "theatres-and-tickets.json"));
