namespace Artillery
{
    ...
    public class StartUp
    {
        public static void Main(string[] args)
        {
            ...
            //1 задача - размаркирай и маркирай другите две
            //ResetDatabase(context, shouldDropDatabase: true);

            var projectDir = GetProjectDirectory();

            //2 задача- размаркирай и маркирай другите две
            //ImportEntities(context, projectDir + @"Datasets/", projectDir + @"ImportResults/");

            //3 задача - размаркирай и маркирай другите две
        ...
        {
            var importCountries = DataProcessor.Deserializer.ImportCountries(context, File.ReadAllText(baseDir + "countries.xml"));
            PrintAndExportEntityToFile(importCountries, exportDir + "Actual Result - ImportCountries.txt");

            var importManufacturers = DataProcessor.Deserializer.ImportManufacturers(context, File.ReadAllText(baseDir + "manufacturers.xml"));
            PrintAndExportEntityToFile(importManufacturers, exportDir + "Actual Result - ImportMnufacturers.txt");

            var importShells = DataProcessor.Deserializer.ImportShells(context, File.ReadAllText(baseDir + "shells.xml"));
            PrintAndExportEntityToFile(importShells, exportDir + "Actual Result - ImportShells.txt");

            var importGuns = DataProcessor.Deserializer.ImportGuns(context, File.ReadAllText(baseDir + "guns.json"));
